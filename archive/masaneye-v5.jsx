...
const PersonChip = ({ personKey, onContact }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 7, background: "#1a1d27", borderRadius: 7, padding: "7px 9px", border: "1px solid #2a2d3a" }}>
      <div style={{ width: 28, height: 28, borderRadius: 6, background: "linear-gradient(135deg,#334155,#475569)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 700, color: "#e2e8f0", fontFamily: "monospace", flexShrink: 0 }}>{p.avatar}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ color: "#e2e8f0", fontSize: 11, fontWeight: 600 }}>{p.name}</div>
...
const ImpactBadge = ({ amount, label }) => amount ? (
  <div style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "#dc262615", border: "1px solid #dc262633", borderRadius: 4, padding: "2px 7px", flexShrink: 0 }}>
    <span style={{ color: "#fca5a5", fontSize: 12, fontWeight: 800, fontFamily: "monospace" }}>{amount}</span>
...
const SourceCard = ({ source, delay, onDone }) => {
  ...
  return (
    <div style={{ background: "#111318", border: `1px solid ${phase === "done" ? source.color + "44" : "#1e2028"}`, borderRadius: 7, padding: "9px 11px", marginBottom: 5, transition: "all 0.3s", opacity: phase === "scan" ? 0.5 : 1 }}>
      ...
      {source.items.slice(0, vis).map((it, i) => (
        <div key={i} style={{ background: "#0a0c10", borderRadius: 5, padding: "5px 8px", marginBottom: 2, borderLeft: `2px solid ${source.color}22`, animation: "fadeSlide 0.2s" }}>
          {source.type === "zalo" ? (<>
            ...
            <div style={{ color: "#cbd5e1", fontSize: 10, lineHeight: 1.3 }}>{it.msg}</div>
          </>) : source.type === "email" ? (<>
            <div style={{ color: "#64748b", fontSize: 8 }}>{it.from}</div>
            <div style={{ color: "#e2e8f0", fontSize: 10, fontWeight: 600 }}>{it.subject}</div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: "#e2e8f0", fontSize: 11, fontWeight: 600 }}>{it.value}</div>
              </div>
              ...
              <span style={{ background: ({ STOCKOUT:"#dc262622",ALERT:"#ea580c22",COMPETITOR:"#7c3aed22",BELOW:"#eab30822",DECLINING:"#eab30822",CRITICAL:"#dc262622",DEGRADED:"#dc262622",BOTTLENECK:"#ea580c22",DELAYED:"#dc262622",AVAILABLE:"#16a34a22",NORMAL:"#16a34a22",INEFFICIENT:"#eab30822",GROWING:"#16a34a22",STABLE:"#33415522",INSIGHT:"#2563eb22",CHURN:"#dc262622",QUALITY:"#dc262622" })[it.flag] || "#33415522",
                color: ({ STOCKOUT:"#fca5a5",ALERT:"#fb923c",COMPETITOR:"#c4b5fd",BELOW:"#fde047",DECLINING:"#fde047",CRITICAL:"#fca5a5",DEGRADED:"#fca5a5",BOTTLENECK:"#fb923c",DELAYED:"#fca5a5",AVAILABLE:"#86efac",NORMAL:"#86efac",INEFFICIENT:"#fde047",GROWING:"#86efac",STABLE:"#94a3b8",INSIGHT:"#60a5fa",CHURN:"#fca5a5",QUALITY:"#fca5a5" })[it.flag] || "#94a3b8",
...
const AnswerBlock = ({ answer, delay, onContact, onAction }) => {
  ...
  if (!vis) return null;
  return (
    <div style={{ background: "linear-gradient(135deg,#0f1923,#111827)", border: "1px solid #1e3a5f", borderRadius: 9, padding: "13px 15px", marginTop: 10, animation: "fadeSlide 0.4s" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
        ...
        <span style={{ color: "#93c5fd", fontWeight: 700, fontSize: 10, fontFamily: "monospace" }}>MasanEye — Synthesis</span>
      </div>
      <div style={{ color: "#e2e8f0", fontSize: 12, lineHeight: 1.4, marginBottom: 10 }}>{answer.summary}</div>
      {answer.points.slice(0, vp).map((p, i) => (
        <div key={i} style={{ background: "#0a0e17", borderRadius: 7, padding: "10px 11px", marginBottom: 7, borderLeft: p.title.includes("🔴") ? "3px solid #ef4444" : "3px solid #eab308", animation: "fadeSlide 0.3s" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 6, marginBottom: 5 }}><div style={{ color: "#f1f5f9", fontSize: 11, fontWeight: 700 }}>{p.title}</div><ImpactBadge amount={p.impact} label={p.impactLabel} /></div>
          <div style={{ color: "#94a3b8", fontSize: 10, lineHeight: 1.5, marginBottom: 7 }}>{p.detail}</div>
          <div style={{ background: "#1e293b", borderRadius: 5, padding: "5px 8px", color: "#67e8f9", fontSize: 10, lineHeight: 1.4, marginBottom: 7 }}>💡 {p.action}</div>
          ...
          {p.actions && <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>{p.actions.map((a, j) => {
            const c = { critical: { bg: "#dc262622", b: "#dc262666", t: "#fca5a5" }, warning: { bg: "#eab30822", b: "#eab30866", t: "#fde047" }, normal: { bg: "#33415522", b: "#33415566", t: "#94a3b8" } }[a.type];
      ...
      {sf && <div style={{ marginTop: 8, padding: "8px 10px", background: "#1e293b", borderRadius: 6, color: "#93c5fd", fontSize: 11, lineHeight: 1.4, animation: "fadeSlide 0.3s" }}>{answer.footer}</div>}
      {sf && answer.entities && (
        <div style={{ background: "#0a0e17", border: "1px solid #1e293b", borderRadius: 7, padding: "8px 10px", marginTop: 8 }}>
          <div style={{ color: "#475569", fontSize: 7, fontFamily: "monospace", marginBottom: 5, fontWeight: 700 }}>ENTITIES</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{answer.entities.map((e, i) => {
            const sc = { critical: "#ef4444", warning: "#eab308", ok: "#22c55e" }[e.status];
            const icons = { dc: "🏭", truck: "🚛", supplier: "📦", competitor: "⚔️", gt: "🏪", product: "🏷️", store: "🏬" };
            return <div key={i} style={{ display: "flex", alignItems: "center", gap: 3, background: "#111827", border: `1px solid ${sc}33`, borderRadius: 4, padding: "2px 7px" }}>
              <span style={{ fontSize: 9 }}>{icons[e.type] || "📋"}</span><span style={{ color: "#cbd5e1", fontSize: 9 }}>{e.name}</span><div style={{ width: 5, height: 5, borderRadius: 3, background: sc }} />
            </div>;
          })}</div>
        </div>
      )}
...
const ZaloGroupList = ({ groups, selectedId, onSelect }) => (
  <div style={{ width: 240, borderRight: "1px solid #1e2028", background: "#0a0c10", overflow: "auto", flexShrink: 0 }}>
    <div style={{ padding: "10px 12px", borderBottom: "1px solid #1a1d27" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
        ...
        <div><div style={{ color: "#e2e8f0", fontSize: 12, fontWeight: 700 }}>Trợ lý Masan</div><div style={{ color: "#4ade80", fontSize: 8, fontFamily: "monospace" }}>● Monitoring</div></div>
      </div>
    </div>
    {groups.map(g => (
      <div key={g.id} onClick={() => onSelect(g.id)} style={{ padding: "8px 12px", cursor: "pointer", borderBottom: "1px solid #111318", background: selectedId === g.id ? "#1e293b" : "transparent", transition: "background 0.15s" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ color: "#e2e8f0", fontSize: 11, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{g.name}</div>
...
const ZaloChatView = ({ group }) => {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#08090c" }}>
      <div style={{ padding: "8px 14px", borderBottom: "1px solid #1e2028", display: "flex", alignItems: "center", gap: 8, background: "#0d0f14" }}>
        ...
        <div><div style={{ color: "#e2e8f0", fontSize: 12, fontWeight: 600 }}>{group.name}</div><div style={{ color: "#475569", fontSize: 9 }}>{group.memberCount} members</div></div>
          ...
          <div key={i} style={{ marginBottom: 6, animation: "fadeSlide 0.15s", display: "flex", gap: 7 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 5, marginBottom: 1 }}>
                <span style={{ color: gc(m.sender), fontSize: 10, fontWeight: 600 }}>{m.sender}</span><span style={{ color: "#475569", fontSize: 8 }}>{m.role}</span><span style={{ color: "#334155", fontSize: 8, marginLeft: "auto" }}>{m.time}</span>
              </div>
              <div style={{ color: m.type === "noise" ? "#475569" : "#cbd5e1", fontSize: 11, lineHeight: 1.3, fontStyle: m.type === "noise" ? "italic" : "normal" }}>{m.msg}</div>
              {m.type === "noise" && <span style={{ fontSize: 7, color: "#334155", fontFamily: "monospace" }}>⊘ NOISE</span>}
            </div>
          </div>
        ))}
      </div>
...
const ProcessingPanel = ({ groupId }) => {
  ...
  if (!ext) return <div style={{ width: 320, background: "#0a0c10", borderLeft: "1px solid #1e2028", display: "flex", alignItems: "center", justifyContent: "center", color: "#334155", fontSize: 11 }}>Select group</div>;

  const uc = { critical: "#ef4444", high: "#f59e0b", medium: "#3b82f6", low: "#22c55e" };
  return (
    <div style={{ width: 320, background: "#0a0c10", borderLeft: "1px solid #1e2028", overflow: "auto", flexShrink: 0 }}>
      <div style={{ padding: "10px 12px", borderBottom: "1px solid #1a1d27" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 4 }}>
          <div style={{ width: 18, height: 18, borderRadius: 4, background: "#548235", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "#fff", fontWeight: 700 }}>H</div>
          <span style={{ color: "#548235", fontSize: 10, fontWeight: 700, fontFamily: "monospace" }}>Haiku + SUPRA/KiotViet Enrichment</span>
        </div>
      ...
      <div style={{ padding: "8px 12px" }}>
        {ext.signals.slice(0, vs).map((s, i) => (
          <div key={i} style={{ background: "#111318", borderRadius: 6, padding: "8px 9px", marginBottom: 5, borderLeft: `3px solid ${uc[s.urgency]}`, animation: "fadeSlide 0.25s" }}>
            ...
            <div style={{ color: "#cbd5e1", fontSize: 10, lineHeight: 1.3, marginTop: 2 }}>{s.summary}</div>
            <div style={{ color: "#334155", fontSize: 8, fontFamily: "monospace", marginTop: 2 }}>conf: {s.confidence}</div>
            {/* SUPRA enrichment */}
            {s.supraMatch && (
              <div style={{ marginTop: 4, padding: "4px 7px", background: "#06b6d411", border: "1px solid #06b6d433", borderRadius: 4 }}>
                ...
                <div style={{ color: "#67e8f9", fontSize: 9, lineHeight: 1.3 }}>{s.supraMatch}</div>
              </div>
            )}
            {/* KiotViet enrichment */}
            {s.kiotMatch && (
              <div style={{ marginTop: 4, padding: "4px 7px", background: "#f59e0b11", border: "1px solid #f59e0b33", borderRadius: 4 }}>
                ...
                <div style={{ color: "#fde68a", fontSize: 9, lineHeight: 1.3 }}>{s.kiotMatch}</div>
              </div>
            )}
          </div>
        ))}
...
const KiotVietTab = () => {
  ...
  const flagColors = { CHURN: "#ef4444", COMPETITOR: "#c4b5fd", QUALITY: "#fb923c", STOCKOUT: "#fca5a5", GROWTH: "#86efac", NEW_DIST: "#67e8f9", STABLE: "#94a3b8", LOW_STOCK: "#fde047" };
  ...
  return (
    <div style={{ flex: 1, overflow: "auto", padding: "14px 20px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        ...
        <div>
          <div style={{ color: "#e2e8f0", fontSize: 16, fontWeight: 700 }}>KiotViet GT Data Partnership</div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 14 }}>
        {Object.entries(KIOTVIET_METRICS.overview).map(([k, v]) => (
          <div key={k} style={{ flex: 1, background: "#111318", borderRadius: 6, padding: "8px 10px", textAlign: "center" }}>
            ...
            <div style={{ color: k === "dataFreshness" ? "#4ade80" : "#e2e8f0", fontSize: 15, fontWeight: 800, fontFamily: "monospace" }}>{v}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {KIOTVIET_METRICS.categories.map((c, i) => (
            <div key={i} style={{ background: "#111318", borderRadius: 6, padding: "8px 10px", marginBottom: 4, display: "flex", alignItems: "center", gap: 8, borderLeft: `3px solid ${trendColors[c.status]}` }}>
              <div style={{ flex: 1 }}>
              ...
              <div style={{ textAlign: "right" }}>
                <div style={{ color: "#e2e8f0", fontSize: 12, fontWeight: 700, fontFamily: "monospace" }}>{c.weeklyRev}</div>
          ...
          {KIOTVIET_METRICS.competitorWatch.map((c, i) => (
            <div key={i} style={{ background: "#111318", borderRadius: 6, padding: "8px 10px", marginBottom: 4, borderLeft: `3px solid ${{ critical: "#ef4444", high: "#f59e0b", low: "#22c55e" }[c.risk]}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                <span style={{ color: "#e2e8f0", fontSize: 11, fontWeight: 600 }}>{c.brand} <span style={{ color: "#475569", fontWeight: 400 }}>vs {c.vs}</span></span>
                <span style={{ color: { critical: "#fca5a5", high: "#fde047", low: "#86efac" }[c.risk], fontSize: 9, fontFamily: "monospace", fontWeight: 700 }}>{c.risk.toUpperCase()} RISK</span>
              </div>
              <div style={{ display: "flex", gap: 12, color: "#94a3b8", fontSize: 9 }}>
                <span>GT Share: <b style={{ color: "#e2e8f0" }}>{c.gtShare}</b></span>
                <span>Trend: <b style={{ color: c.trend.includes("↑") ? "#fca5a5" : c.trend.includes("↓") ? "#86efac" : "#94a3b8" }}>{c.trend}</b></span>
        ...
        <div style={{ width: 360, flexShrink: 0 }}>
          ...
          {KIOTVIET_METRICS.realtimeFeed.slice(0, feedVis).map((f, i) => (
            <div key={i} style={{ background: "#111318", borderRadius: 6, padding: "7px 9px", marginBottom: 3, animation: "fadeSlide 0.25s", borderLeft: `2px solid ${flagColors[f.flag]}33` }}>
              ...
              <div style={{ color: "#cbd5e1", fontSize: 10, lineHeight: 1.3 }}>{f.event}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Value proposition */}
      <div style={{ marginTop: 16, background: "#0f1923", border: "1px solid #1e3a5f", borderRadius: 8, padding: "12px 14px" }}>
        <div style={{ color: "#93c5fd", fontSize: 11, fontWeight: 700, marginBottom: 6 }}>💡 Why this matters — data we'd NEVER see without KiotViet</div>
        <div style={{ display: "flex", gap: 8 }}>
          {[
            ...
            <div key={i} style={{ flex: 1, background: "#111827", borderRadius: 6, padding: "8px 10px" }}>
              <div style={{ fontSize: 16, marginBottom: 4 }}>{v.icon}</div>
              <div style={{ color: "#e2e8f0", fontSize: 10, fontWeight: 600, marginBottom: 3 }}>{v.title}</div>
...
export default function MasanEyeDemo() {
  ...
  // Auto-scroll only during animation phase, then stop so user can scroll freely
  useEffect(() => {
    if (scenario) {
      autoScrollRef.current = true;
      // Calculate total animation duration based on sources + answer
      const sc = SCENARIOS[scenario];
      const totalDuration = sc ? (sc.sources.length * 1400 + 800 + sc.answer.points.length * 1200 + 2000) : 15000;
      const iv = setInterval(() => {
        if (autoScrollRef.current && scrollRef.current) {
          scrollRef.current.scrollTop = scrollRef.current.scrollHeight;
        }
      }, 100);
      // Stop auto-scroll after animation completes
      scrollTimerRef.current = setTimeout(() => {
        autoScrollRef.current = false;
        clearInterval(iv);
      }, totalDuration);
      // Also stop if user manually scrolls up
      const handleScroll = () => {
  ...
  return (
    <div style={{ background: "#08090c", height: "100vh", fontFamily: "'Segoe UI',system-ui,sans-serif", color: "#e2e8f0", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <style>{`
      ...
      {/* HEADER */}
      <div style={{ padding: "8px 18px", borderBottom: "1px solid #1e2028", display: "flex", alignItems: "center", gap: 8, background: "#0d0f14", flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: 24, display: "flex", gap: 2, background: "#111318", borderRadius: 6, padding: 2 }}>
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              ...
              background: tab === t.id ? "#1e293b" : "transparent", color: tab === t.id ? "#e2e8f0" : "#64748b",
            ...
            {!sc ? (
              <div style={{ animation: "fadeSlide 0.4s" }}>
                ...
                {Object.entries(SCENARIOS).map(([k, s]) => (
                  <button key={k} onClick={() => setScenario(k)} style={{ display: "block", width: "100%", textAlign: "left", background: "#111318", border: "1px solid #1e2028", borderRadius: 6, padding: "9px 13px", marginBottom: 4, color: "#cbd5e1", fontSize: 12, cursor: "pointer", fontFamily: "inherit", transition: "all 0.15s" }}
                    onMouseEnter={e => e.target.style.borderColor = "#2563eb"} onMouseLeave={e => e.target.style.borderColor = "#1e2028"}>
                    "{s.query}"
                  </button>
                ))}
              </div>
              ...
              <div style={{ animation: "fadeSlide 0.3s" }}>
                <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 10 }}><div style={{ background: "#1e3a5f", borderRadius: "10px 10px 2px 10px", padding: "8px 13px", maxWidth: "70%", fontSize: 12 }}>{sc.query}</div></div>
                ...
                <AnswerBlock answer={sc.answer} delay={sc.sources.length * 1400 + 800} onContact={(m, p) => setContactModal({ method: m, person: p })} onAction={l => setToast(l)} />
                <button onClick={() => setScenario(null)} style={{ marginTop: 10, background: "transparent", border: "1px solid #1e2028", borderRadius: 4, padding: "4px 10px", color: "#64748b", fontSize: 9, cursor: "pointer", fontFamily: "monospace" }}>← Back</button>
              </div>
            )}
          </div>
          <div style={{ padding: "7px 18px", borderTop: "1px solid #1e2028", background: "#0a0c10", flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "center", background: "#111318", border: "1px solid #1e2028", borderRadius: 7, padding: "0 12px" }}>
              <input value={inputVal} onChange={e => setInputVal(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && inputVal.trim()) { const l = inputVal.toLowerCase(); if (l.includes("chin") || l.includes("gt")) setScenario("chinsu_gt"); else if (l.includes("margin") || l.includes("meatdeli")) setScenario("meatdeli_margin"); else setScenario("d2_ops"); setInputVal(""); } }}
                placeholder="Hỏi gì cũng được... (thử: 'Chin-su GT miền Nam?')" style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: "#e2e8f0", fontSize: 12, padding: "9px 0", fontFamily: "inherit" }} />
      ...
      {/* Modal */}
      {contactModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }} onClick={() => setContactModal(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#1a1d27", border: "1px solid #2a2d3a", borderRadius: 10, padding: 18, minWidth: 280 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: 8, background: "linear-gradient(135deg,#334155,#475569)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#e2e8f0", fontFamily: "monospace" }}>{contactModal.person.avatar}</div>
              <div><div style={{ color: "#f1f5f9", fontSize: 14, fontWeight: 700 }}>{contactModal.person.name}</div><div style={{ color: "#64748b", fontSize: 10 }}>{contactModal.person.role}</div></div>
            </div>
            {[{ l: "Phone", v: contactModal.person.phone }, { l: "Email", v: contactModal.person.email }].map((b, i) => (
              <div key={i} style={{ background: "#111318", border: "1px solid #2a2d3a", borderRadius: 6, padding: "7px 10px", marginBottom: 5, display: "flex", alignItems: "center", gap: 7 }}>
                <span style={{ fontSize: 14 }}>{i === 0 ? "📞" : "✉️"}</span>
                <div><div style={{ color: "#94a3b8", fontSize: 8 }}>{b.l}</div><div style={{ color: "#e2e8f0", fontSize: 12, fontFamily: "monospace" }}>{b.v}</div></div>
              </div>
            ))}
...
function Ticker() {
  ...
  return <div style={{ background: "#0a0c10", borderBottom: "1px solid #1a1d27", padding: "4px 18px", height: 22, flexShrink: 0, overflow: "hidden" }}><div style={{ color: "#94a3b8", fontSize: 9, fontFamily: "monospace", whiteSpace: "nowrap" }}>{a[t % a.length]}</div></div>;
}
